...
export const GoalCardModal: React.FC<GoalCardModalProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const [step, setStep] = useState(1); // 1: 활동 선택, 2: Request B (목표 설정 포기인 경우) or 내용 작성, 3: 내용 작성 (목표 설정 포기인 경우)
  ...
  useEffect(() => {
    if (step === 2 && selectedActivity === '목표 설정 포기' && requestBButtonRef.current) {
      setTimeout(() => {
      ...
      >
        ...
        <div className="relative w-full max-h-[95vh]" style={{
          ...
          <div className={`flex flex-col ${isMobile ? 'relative' : 'absolute inset-0'}`} style={{
            ...
            {/* Step indicator */}
            <div className={`relative flex items-center justify-center gap-[46%] px-[27%] ${isMobile ? 'hidden' : ''}`} style={{
              ...
              <div className="flex flex-col items-center gap-[clamp(4px,1vw,8px)] relative z-10">
                <div className="relative flex items-center justify-center">
                  <div className={`rounded-full ${(selectedActivity !== '목표 설정 포기' && step === 2) || (selectedActivity === '목표 설정 포기' && step === 3) ? 'bg-[#21e786]' : 'bg-[#888888]'} opacity-20`} style={{ width: 'clamp(20px,5vw,30px)', height: 'clamp(20px,5vw,30px)' }} />
                  <div className={`absolute rounded-full ${(selectedActivity !== '목표 설정 포기' && step === 2) || (selectedActivity === '목표 설정 포기' && step === 3) ? 'bg-[#21e786]' : 'bg-[#888888]'}`} style={{ width: 'clamp(10px,2.5vw,15px)', height: 'clamp(10px,2.5vw,15px)' }} />
                </div>
                <span className={`font-ria-sans font-medium ${(selectedActivity !== '목표 설정 포기' && step === 2) || (selectedActivity === '목표 설정 포기' && step === 3) ? 'text-white' : 'text-[#888888]'} whitespace-nowrap`} style={{ fontSize: isTablet ? '12px' : 'clamp(12px,3vw,16px)' }}>
                  내용 작성
                </span>
              </div>
            </div>
            ...
            <div className={`relative ${isMobile ? 'mx-[5%]' : 'mx-[17.7%]'}`} style={{
              ...
              {/* Content overlay */}
              <div
                className={`${isMobile ? 'flex flex-col px-3 py-2.5 overflow-y-auto custom-scrollbar' : 'absolute inset-0 flex flex-col overflow-y-auto custom-scrollbar'} ${isTablet ? 'px-[5%] py-[4%]' : isMobile ? '' : 'px-[5%] py-[6%]'}`}
                style={{
                  scrollbarWidth: 'thin',
                  scrollbarColor: '#21e786 transparent',
                  maxHeight: isMobile ? (step === 2 ? '280px' : '290px') : undefined
                }}
              >
                {step === 1 ? (
                  <>
                    ...
                    <h3 className="font-ria-sans font-bold text-[#21e786]" style={{
                      ...
                      marginBottom: isMobile ? '3px' : isTablet ? '2%' : '3%'
                    }}>
                      Request A
                    </h3>
                    ...
                    <p className="[font-family:'Pretendard',sans-serif] font-medium text-white" style={{
                      ...
                      marginBottom: isMobile ? '4px' : isTablet ? '1.5%' : '2%'
                    }}>
                      {isMobile
                        ? '앞으로 일주일 동안 예정된 클럽 활동 중에서 \'반드시\' 성취하고자 하는 그대의 목표를 1개 설정해보세요!'
                        : '앞으로 일주일 동안 예정된 클럽 활동 중에서 \'반드시\' 성취하고자 하는 그대의 목표를 1개 설정해보세요!'}
                      {!isMobile && (
                        <>
                          <br />
                          목표를 설정할 수 있는 상황이 아닌 경우에는 '목표 설정 포기'를 선택할 수 있습니다.
                        </>
                      )}
                    </p>
                    ...
                    <p className="[font-family:'Pretendard',sans-serif] font-normal text-[#666666]" style={{
                      ...
                      marginBottom: isMobile ? '8px' : isTablet ? '15px' : '20px'
                    }}>
                    ...
                    <div className="relative">
                      <div
                        ...
                        style={{
                          ...
                          marginBottom: isMobile ? '5px' : isTablet ? '20px' : '30px'
                        }}
                        ...
                        >
                          <div className={isMobile ? 'flex flex-col items-center gap-2' : 'flex flex-wrap gap-[2%]'}>
                            {activityOptions.map((option, index) => {
                              ...
                              const borderColor = option === '목표 설정 포기' ? 'border-white' : 'border-[#21e786]';
                              const isNoRegret = option === '목표 설정 포기';
                ...
                ) : step === 2 && selectedActivity === '목표 설정 포기' ? (
                  <>
                    {/* Step 2: Request B (목표 설정 포기인 경우) */}
                    {/* Selected Activity */}
                    ...
                    <div className={`${isMobile ? 'mb-3' : 'mb-[3%]'}`}>
                      ...
                      <p className="[font-family:'Pretendard',sans-serif] font-medium text-[#555555]" style={{
                        ...
                        위 '목표' 활동을 택한 경우, 앞으로의 1주일의 클럽 활동 중 이 활동을 '목표'로 선정한 이유와 1주일 후 해당 목표의 달성 여부를 판가름 하는 시점에 있을 미래의 본인에게 보내는 각오 메시지를 작성해주세요!
                      </p>
                      {!isMobile && (
                        <p className="[font-family:'Pretendard',sans-serif] font-normal text-[#555555] leading-[1.5]" style={{
                          fontSize: isTablet ? '11px' : '14px'
                        }}>
                          *본 과정을 통해, 미래 1주일 후의 스스로에게 목표와 다짐을 전달하며 동기부여를 다잡고 지속적인 성장의 기틀을 마련할 수 있습니다. 목표를 세우지 않으면 실패도/성공도 할 수 없어!
                        </p>
                      )}
                    </div>
                  ...
                  <>
                    ...
                    <div className={`${isMobile ? 'mb-2' : 'mb-[4%]'}`}>
                      <h3 className="font-ria-sans font-bold text-[#21e786]" style={{
                        ...
                        {selectedActivity === '목표 설정 포기' ? 'Request C' : 'Request B'}
                      </h3>
                      {selectedActivity === '목표 설정 포기' ? (
                        // 목표 설정 포기 선택 시 - Request C
                        <>
                          <p className="[font-family:'Pretendard',sans-serif] font-medium text-white" style={{
                            fontSize: isMobile ? '14px' : isTablet ? '14px' : '18px',
                            lineHeight: isMobile ? '1.4' : '1.5',
                            ...
                            위 '목표 설정' 활동을 포기한 경우, 별다른 목표를 설정할 수 없는 개인의 이유와 그래도 소중한 우리의 이번 한주에 대해 나름의 의미를 부여해주세요! 한걸음의 휴식은 두 걸음의 질주를 위한 것!
                          </p>
                          {!isMobile && (
                          <p className="[font-family:'Pretendard',sans-serif] font-normal text-[#666666] leading-[1.5]" style={{
                            fontSize: isTablet ? '11px' : '14px'
                          }}>
                            *서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트서브멘트
                          </p>
                          )}
                        </>
                      ) : (
                        ...
                        <>
                          ...
                          <p className="[font-family:'Pretendard',sans-serif] font-normal text-[#666666] leading-[1.5]" style={{
                            ...
                            *본 과정을 통해, 미래 1주일 후의 스스로에게 목표와 다짐을 전달하며 동기부열르 다잡고 지속적인 성장의 기틀을 마련할 수 있습니다. 목표를 세우지 않으면 실패도/성공도 할 수 없어!
                          </p>
                          )}
                        </>
                      )}
                    ...
                    {/* Star Rating */}
                    <div className={`${isMobile ? 'mb-1.5' : 'mb-[3%]'}`} style={{
                      ...
                      <div className="flex gap-0 justify-start">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <button
                            ...
                            className={`transition-all hover:scale-110 ${isMobile ? 'px-0.5 py-1.5' : 'p-0'}`}
                          >
                            <svg
      ...
      {showNotice && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm">
          <div className="animate-fade-in">
            <img
              src={isMobile ? "/noticeModal-m4.png" : "/noticeModal02.png"}
              alt="목표 작성 완료"
